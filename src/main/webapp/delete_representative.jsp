...
if (selectedRepId != null && !selectedRepId.isEmpty()) {
    ...
    if (rowsAffected > 0) {
        out.println("<script>alert('Representative with ID: " + selectedRepId + " deleted successfully.');</script>");
    } else {
        out.println("<script>alert('Failed to delete representative with ID: " + selectedRepId + ".');</script>");
    }
}

Statement st1 = con.createStatement();
ResultSet rs1 = st1.executeQuery("SELECT username FROM users WHERE account_type = 'customer_rep'");

%>
<!DOCTYPE html>
<html>
<head>
    <title>Delete a Customer Representative</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>Delete a Customer Representative</h1>
        <a href='logout.jsp'>Log out</a>
        <a href='manage_customer_reps.jsp'>Back</a>
        <form action="delete_representative.jsp" method="post">
            <select name="repId">
                <% while (rs1.next()) { %>
